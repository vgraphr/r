unless module.hot
  ...
  unless window.requestAnimationFrame
    ...
    window.cancelAnimationFrame = (id) ->
      clearTimeout id
  else unless window.cancelAnimationFrame
    prevRequestAnimationFrame = window.requestAnimationFrame
    window.requestAnimationFrame = (callback, element) ->
      id = lastId++
      requests[id] = true
      prevRequestAnimationFrame (->
        if requests[id]
          callback()
      ), element
      return myId
...
AnimationUpdater =
  ...
  addAll: (list) ->
    AnimationUpdater.elements.concat list

  add: (object) ->
    AnimationUpdater.elements.push object

  run: ->
    if AnimationUpdater.elements.some((element) -> element.update())
      AnimationUpdater.animationId = requestAnimationFrame AnimationUpdater.run
    else
      cancelAnimationFrame AnimationUpdater.animationId
...
formatNumber = (num) -> addCommas num.toFixed 0
